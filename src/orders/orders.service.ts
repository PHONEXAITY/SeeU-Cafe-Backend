...
export class OrdersService {
  ...
  /*  async create(createOrderDto: CreateOrderDto): Promise<OrderWithRelations> {
    ...
    // ส่ง Webhook ไป n8n
    try {
      // First, get the user name if User_id exists
      const userName = createOrderDto.User_id
        ? await this.prisma.user
            .findUnique({ where: { id: createOrderDto.User_id } })
            .then(
              (user) =>
      ...
      await this.httpService
        .post(
          process.env.N8N_WEBHOOK_URL ||
            'https://your-n8n-instance/webhook/new-order',
          webhookPayload,
          {
            headers: {
      ...
      console.log('Webhook sent to n8n:', webhookPayload);
    } catch (webhookError) {
      console.error('Failed to send webhook to n8n:', webhookError.message);
      // Don't let webhook failure affect order creation
    }
